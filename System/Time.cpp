...
{
	QueryPerformanceFrequency((LARGE_INTEGER *)&ticksPerSecond); //cpu clock? time?
...
void Time::Create() //timer need call
{
	assert(instance == NULL);
...
void Time::Update()
{
	...
	QueryPerformanceCounter((LARGE_INTEGER *)&currentTime); //cpu clock? time? 성능
